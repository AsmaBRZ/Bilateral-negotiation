...
public class ImitatingAgent extends Agent {
	...
	private double doublelastPartnerBid;
	private double lastOppBid=-1;
	private double my_last_bid=-1;
        private  int cp=0;
 	/**
	...
	public Action chooseAction() {
		Action action = null;
double offeredUtilFromOpponent=-1;
		try {
			if (actionOfPartner == null)
				action = chooseRandomBidAction();
			else if (actionOfPartner instanceof Offer) {
			        
				offeredUtilFromOpponent = getUtility(lastPartnerBid);
				if(cp==0){
					this.lastOppBid=offeredUtilFromOpponent;
					this.doublelastPartnerBid=offeredUtilFromOpponent;
					cp++;
				}
				// get current time
	...
	private Bid getRandomBid() throws Exception {
		...
		Random randomnr = new Random();
		        ...
		        if(this.doublelastPartnerBid>this.lastOppBid){
		        	do {
					for (Issue lIssue : issues) {
						switch (lIssue.getType()) {
						case DISCRETE:
					...
					bid = new Bid(utilitySpace.getDomain(), values);
				} while (getUtility(bid) <this.my_last_bid);
		        }else{
		        	do {
					for (Issue lIssue : issues) {
						switch (lIssue.getType()) {
                ...
                this.my_last_bid=getUtility(bid);
		return bid;
	}
