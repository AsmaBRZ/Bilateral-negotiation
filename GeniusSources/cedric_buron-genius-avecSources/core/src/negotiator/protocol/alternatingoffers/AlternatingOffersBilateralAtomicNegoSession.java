...
public class AlternatingOffersBilateralAtomicNegoSession extends BilateralAtomicNegotiationSession {
	...
	public AlternatingOffersBilateralAtomicNegoSession(Protocol protocol, Agent agentA, Agent agentB, String agentAname,
	...
	public void run() {
		...
		try {
			...
			agentA.internalInit(protocol.getSessionNumber(), protocol.getTotalSessions(), startTime, totalTime,
					timeline, (AbstractUtilitySpace) spaceA.copy(), agentAparams, new AgentID("agentA"));
			agentA.init();

			agentB.internalInit(protocol.getSessionNumber(), protocol.getTotalSessions(), startTime, totalTime,
					timeline, (AbstractUtilitySpace) spaceB.copy(), agentBparams, new AgentID("agentB"));
			agentB.init();
				...
				try {
					...
					if (timeline.isDeadlineReached()) {
						...
						if (hasMAC()) {
							if (agentAWithMultiAC) {
								...
								((Multi_AcceptanceCondition) AC).remainingAC("deadline");
							}
					...
					if (lastAction instanceof EndNegotiation) {
						...
						if (hasMAC()) {
							if (agentAWithMultiAC) {
								AcceptanceStrategy AC = ((BOAagent) agentA).getAcceptanceStrategy();
								((Multi_AcceptanceCondition) AC).remainingAC("breakoff");
							}
							if (agentBWithMultiAC) {
								AcceptanceStrategy AC = ((BOAagent) agentB).getAcceptanceStrategy();
								((Multi_AcceptanceCondition) AC).remainingAC("breakoff");
							...
							badOutcome(time, "Agent [" + currentAgent.getName()
						...
						if (showGUI) {
							...
							fireLogMessage("Nego", "Utility of " + agentA.getName() + "(A): " + utilA);

							fireLogMessage("Nego", "Utility of " + agentB.getName() + "(B): " + utilB);
						}
						// save last results

						BidPointTime p = null;
						p = new BidPointTime(lastBid, spaceA.getUtility(lastBid), spaceB.getUtility(lastBid), time);
						...
						if (showGUI) {
							fireNegotiationActionEvent(currentAgent, lastAction, finalRound,
									System.currentTimeMillis() - startTimeMillies, time, utilA, utilB,
									agentAUtilityDisc, agentBUtilityDisc, "bid by " + currentAgent.getName(), false);
						}
						...
						if (agentAWithMultiAC && acceptedBy.equalsIgnoreCase("agentB")) {
							AcceptanceStrategy AC = ((BOAagent) agentA).getAcceptanceStrategy();
							((Multi_AcceptanceCondition) AC).remainingACAccept(lastBid, time, fAgentABids, fAgentBBids,
									acceptedBy);
					...
					String exceptionMsg = "Caught exception. Agent [" + currentAgent.getName()
							+ "] crashed. Last known action = " + lastAction + ". Emergency outcome: (rvA, rvB)."
							+ ". Details: " + e.toString();
					try {
						// checkForMAC();
						if (!hasMAC()) {
			...
			if (e instanceof ThreadDeath) {
				...
				// Main.logger.add("Negotiation was timed out. Both parties get
				// util=0");
		...
		for (ArrayList<OutcomeTuple> agentList : completeList) {
			for (OutcomeTuple outcomeTuple : agentList) {
				...
				if (outcomeTuple.getLogMsgType() == "deadline") {
					...
					logMsg = "Agent [" + currentAgent.getName()
							+ "] sent EndNegotiation, so the negotiation ended without agreement";
					badOutcome(time, logMsg);
					changeNameofAC(agentAWithMultiAC, agentBWithMultiAC, outcomeTuple);
					MACoutcomes.add(no);
	 ...
	 */
	private void newOutcome(Agent currentAgent, double utilA, double utilB, double utilADiscount, double utilBDiscount,
			String message, double time, double distanceToNash, String acceptedBy) throws Exception {
		...
		try {
			...
			System.err.println(
					agentA.getName() + " crashed in endSession. This may influence the agent's future behavior.");
		}
		...
		try {
			...
			System.err.println(
					agentB.getName() + " crashed in endSession. This may influence the agent's future behavior.");
		}
		OutcomeInfo outcomeInfo = new OutcomeInfo(agentA.getName(), agentB.getName(),
				agentA.getClass().getCanonicalName(), agentB.getClass().getCanonicalName(), utilA, utilB, utilADiscount,
				utilBDiscount, message, 1.0, 1.0, spaceA.getDomain().getName(), spaceA.getFileName(),
				spaceB.getFileName(), time, acceptedBy);

		no = new NegotiationOutcome(this, this.protocol.getSessionNumber(), lastAction, fAgentABids, fAgentBBids,
				startingWithA, additionalLog, distanceToNash, outcomeInfo);
		...
		if (showGUI) {
			fireNegotiationActionEvent(currentAgent, lastAction, fAgentABids.size() + fAgentBBids.size(),
					System.currentTimeMillis() - startTimeMillies, time, utilA, utilB, utilADiscount, utilBDiscount,
					message, true);
		}
	}
	 ...
	 **/
	private void newOutcome(Agent currentAgent, Action lastAction, double utilA, double utilB, double utilADiscount,
			double utilBDiscount, String message, ArrayList<BidPointTime> agentASize,
			ArrayList<BidPointTime> agentBSize, double time, double distanceToNash, String acceptedBy)
			throws Exception {
		OutcomeInfo outcomeInfo = new OutcomeInfo(agentA.getName(), agentB.getName(),
				agentA.getClass().getCanonicalName(), agentB.getClass().getCanonicalName(), utilA, utilB, utilADiscount,
				utilBDiscount, message, 1.0, 1.0, spaceA.getDomain().getName(), spaceA.getFileName(),
		...
		no = new NegotiationOutcome(this, this.protocol.getSessionNumber(), lastAction,
				(ArrayList<BidPointTime>) agentASize, (ArrayList<BidPointTime>) agentBSize, startingWithA,
		...
		if (TournamentConfiguration.getBooleanOption("logFinalAccuracy", false)) {
			if (agentA instanceof BOAagent) {
				...
				if (!(opponentModel instanceof NoModel)) {
					...
					AbstractUtilitySpace estimatedOpponentUS = opponentModel.getOpponentUtilitySpace();
					BidSpace estimatedBS = null;
					try {
						estimatedBS = new BidSpace(spaceA, estimatedOpponentUS, false, true);
					} catch (Exception e) {
						e.printStackTrace();
					}
					omMeasuresResults.addPearsonCorrelationCoefficientOfBids(
							omMeasures.calculatePearsonCorrelationCoefficientBids(estimatedOpponentUS));
					omMeasuresResults
							.addRankingDistanceOfBids(omMeasures.calculateRankingDistanceBids(estimatedOpponentUS));
					omMeasuresResults.addRankingDistanceOfIssueWeights(
							omMeasures.calculateRankingDistanceWeights(opponentModel));
					omMeasuresResults
							.addAverageDifferenceBetweenBids(omMeasures.calculateAvgDiffBetweenBids(opponentModel));
					omMeasuresResults.addAverageDifferenceBetweenIssueWeights(
							omMeasures.calculateAvgDiffBetweenIssueWeights(opponentModel));
					...
					omMeasuresResults.addAverageDifferenceOfParetoFrontier(
							omMeasures.calculateAvgDiffParetoBidToEstimate(estimatedOpponentUS));
					omMeasuresResults.addPercentageOfCorrectlyEstimatedParetoBids(
							omMeasures.calculatePercCorrectlyEstimatedParetoBids(estimatedBS));
					omMeasuresResults.addPercentageOfIncorrectlyEstimatedParetoBids(
							omMeasures.calculatePercIncorrectlyEstimatedParetoBids(estimatedBS));
					omMeasuresResults
							.addParetoFrontierDistance(omMeasures.calculateParetoFrontierDistance(estimatedBS));
					estimatedBS = null;
	...
	private void processDataForLogging(double time, boolean agreement) {
		...
		if (TournamentConfiguration.getBooleanOption("logNegotiationTrace", false)) {
			...
			matchDataLogger.writeToFileCompact(time, agreement, protocol.getSessionNumber());
		}
	}
				...
				for (ArrayList<OutcomeTuple> outcomeTupleList : completeList)
					for (OutcomeTuple outcomeTuple : outcomeTupleList) {
						if (outcomeTuple.getLogMsgType().equalsIgnoreCase("Judge Timeout")) {
							newOutcome(currentAgent, rvA, rvB, rvADiscounted, rvBDiscounted,
									"JudgeTimeout: negotiation was timed out", time, distanceToNash, "");
							changeNameofAC(agentAWithMultiAC, agentBWithMultiAC, outcomeTuple);
	...
	public void createOutcome(Bid lastBid, double time, boolean isMac, OutcomeTuple outcomeTuple, String acceptedBy)
			throws Exception {
		...
		if (isMac) {
			...
			if (outcomeTuple == null) {
				newOutcome(currentAgent, agentAUtility, agentBUtility, agentAUtilityDisc, agentBUtilityDisc, null, time,
				...
				if (fAgentABids.size() == outcomeTuple.getAgentASize()
					...
					subAgentABids = new ArrayList<BidPointTime>(
							fAgentABids.subList(0, outcomeTuple.getAgentASize() + 1));
					subAgentBBids = new ArrayList<BidPointTime>(
							fAgentBBids.subList(0, outcomeTuple.getAgentBSize() + 1));
				}
				newOutcome(currentAgent, new Accept(agentB.getAgentID(), lastBid), agentAUtility, agentBUtility,
						agentAUtilityDisc, agentBUtilityDisc, null, subAgentABids, subAgentBBids,
						outcomeTuple.getTime(), distanceToNash, outcomeTuple.getAcceptedBy());
				changeNameofAC(agentAWithMultiAC, agentBWithMultiAC, outcomeTuple);
				MACoutcomes.add(no);
			}
